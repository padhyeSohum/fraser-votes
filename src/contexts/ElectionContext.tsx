...
interface ElectionContextType {
    ...
    updateCandidate: (
        id: string,
        candidate: Partial<Omit<Candidate, "id" | "votes">>
    ) => Promise<void>;
    ...
    updatePosition: (
        id: string,
        position: Partial<Omit<Position, "id">>
    ) => Promise<void>;
    removePosition: (id: string) => Promise<void>;

    // Student methods
    addStudent: (
        student: Omit<Student, "id" | "checkedIn" | "hasVoted">
    ) => Promise<void>;
    addStudents: (
        students: Omit<
            Student,
            "id" | "checkedIn" | "checkedInBy" | "checkedInAt" | "hasVoted"
        >[]
    ) => Promise<void>;
    updateStudent: (
        id: string,
...
export const ElectionProvider: React.FC<{ children: React.ReactNode }> = ({
    children,
}) => {
        ...
        const setupFirebaseListeners = async () => {
            try {
                ...
                if (err.code === "permission-denied") {
                    setOfflineMode(true);
                    const storedCandidates = getFromLocalStorage<Candidate[]>(
                        "candidates",
                        []
                    );
                    const storedPositions = getFromLocalStorage<Position[]>(
                        "positions",
                        []
                    );
                    const storedStudents = getFromLocalStorage<Student[]>(
                        "students",
                        []
                    );
                    const storedSettings =
                        getFromLocalStorage<ElectionSettings>(
                            "settings",
                            settings
                        );
                    ...
                    toast({
                        ...
                        description:
                            "Firebase permissions are restricted. Using local storage for data persistence.",
                        variant: "destructive",
                    });
                } else {
        ...
        try {
            ...
            if (err.code === "permission-denied") {
                ...
                toast({
                    title: "Limited Access Mode",
                    description:
                        "Student check-in status has been reset (local only)",
                });
            } else {
                setError(err.message);
                toast({
        ...
        try {
            ...
            if (err.code === "permission-denied") {
                ...
                toast({
                    title: "Limited Access Mode",
                    description:
                        "Election settings updated successfully (local only)",
                });
            } else {
                setError(err.message);
                toast({
        ...
        try {
            ...
            if (err.code === "permission-denied") {
                ...
                toast({
                    title: "Limited Access Mode",
                    description:
                        "Your vote has been recorded successfully (local only)",
                });
                return;
            }
